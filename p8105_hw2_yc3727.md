...
#### Description of the NYC Transit dataset
  ...
  - It contains the following variables: ada, entrance\_type, entry,
    line, route1, route10, route11, route2, route3, route4, route5,
    route6, route7, route8, route9, station\_latitude,
...
``` r
#Question 1: number of distinct stations
  distinct_transit_df = 
    distinct(transit_df, line, station_name, .keep_all = TRUE) 

#Question 2: number of ADA compliant stations
  ada_comp_df = 
    distinct(transit_df, line, station_name, .keep_all = TRUE) %>% 
    filter(ada == TRUE)
  
#Question 3: proportion of entrances without vending
   numerator_df = distinct_transit_df %>%
     filter(vending == "NO", entry == TRUE)
   
   no_vending_ratio = 
     nrow(numerator_df)/nrow(filter(distinct_transit_df, vending == "NO"))
...
```
  ...
  - 69 out of 183 station entrances/exits without vending allow
    entrance.
